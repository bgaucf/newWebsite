...
function ExtracurricularsSection() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6">
      ...
      >
        <h2 className="text-2xl md:text-3xl font-bold">Extracurriculars</h2>
      </div>
...
function ExtracurricularCard({ item, index }) {
  const [ref, isVisible] = useScrollReveal();
  const baseClass = 'rounded-xl p-6 bg-cream-dark border border-gray-200';
  const clickableClass = 'hover:border-gold/50 transition-colors hover:shadow-md';
  const content = (
    <>
  ...
  return item.path ? (
    <Link ref={ref} to={item.path} className={`block ${cardClassName} ${clickableClass} group`} style={cardStyle}>
      {content}
    </Link>
  ) : (
